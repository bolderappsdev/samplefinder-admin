 ...
 */
export function hasRecentBadgeAward(
  profile: UserProfile,
  badgeType: 'ambassador' | 'influencer',
  now: number
): boolean {
  const raw = (profile as unknown as { notifications?: unknown }).notifications;
  let list: unknown[] = [];
  if (Array.isArray(raw)) {
    list = raw;
  } else if (typeof raw === 'string' && raw) {
  ...
  for (const item of list) {
    ...
    if (typeof item === 'string') {
      try {
        entry = JSON.parse(item) as Record<string, unknown>;
      } catch {
        entry = null;
      }
    ...
    if (!entry || entry.type !== 'badgeEarned') continue;
    const data = entry.data as Record<string, unknown> | undefined;
    if (data?.badgeType !== badgeType) continue;
    const createdAt = Date.parse(String(entry.createdAt ?? ''));
    if (!Number.isFinite(createdAt)) continue;
    if (now - createdAt < BADGE_AWARD_IDEMPOTENCY_WINDOW_MS) return true;
  }
  return false;
}
...
export async function sendBadgeNotification(
  ...
  const serverAwardsPoints = isSettingEnabled(
    await getSettingValue(databases, SETTING_SERVER_AWARDS_BADGE_POINTS)
  );
  if (!serverAwardsPoints) {
    log(
      `Badge notification: ${badgeType} points NOT awarded — "${SETTING_SERVER_AWARDS_BADGE_POINTS}" is not enabled, so the mobile app is still the payer. Enable it once the release that removes the client-side award has adoption.`
    );
    return result;
  }
  ...
  if (alreadyAwarded) {
    log(
      `Badge notification: ${badgeType} points NOT awarded to ${profile.$id} — an award for this badge was already recorded within the idempotency window`
    );
    return result;
  }
  ...
  try {
    ...
    log(
      `Badge notification: awarded ${SPECIAL_BADGE_POINTS} ${badgeType} points to ${profile.$id}; totalPoints ${currentPoints} -> ${currentPoints + SPECIAL_BADGE_POINTS}`
    );
  } catch (err: unknown) {
